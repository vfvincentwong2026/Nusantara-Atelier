import type { RoomType } from './types';

export type Locale = 'zh' | 'en' | 'id';

export const LOCALES: { code: Locale; label: string }[] = [
  { code: 'id', label: 'ID' },
  { code: 'en', label: 'EN' },
  { code: 'zh', label: '中文' },
];

export const STORAGE_KEY = 'na-locale';

const zh = {
  nav: {
    services: '服务',
    cases: '案例',
    pricing: '造价',
    team: '团队',
    materials: '材料库',
    book: '预约设计师',
  },
  hero: {
    eyebrow: 'Nusantara Atelier · 豪宅全案服务商',
    titleA: '从设计到呈现，',
    titleB: '一步到位。',
    tagline: 'From Design to Delivery — Done Right, Once.',
    desc: '中国顶级豪宅落地经验，服务印尼高端别墅市场。',
    descChain: '室内设计 · 精密装修 · 家具软装指导',
    ctaPrimary: '预约设计师',
    ctaSecondary: '浏览案例库',
    scroll: '向下探索 ↓',
  },
  services: {
    eyebrow: 'Our Services',
    title: '一条完整的服务链条',
    items: [
      {
        icon: '🏛️',
        title: '室内设计',
        subtitle: 'Interior Design',
        desc: '基于真实落地案例的风格体系：法式、现代、侘寂、意式极简、现代奶油、法式轻奢。',
        backup: '每一套方案都有真实成交项目作为依据。',
      },
      {
        icon: '🔨',
        title: '精密装修',
        subtitle: 'Precision Fit-Out',
        desc: '硬装造价透明到每一平方米。真实案例单方造价 4,500–5,500 元/㎡ 硬装标准。',
        backup: '工艺对标中国一线豪宅。',
      },
      {
        icon: '🛋️',
        title: '家具软装指导',
        subtitle: 'Furniture & Soft Furnishing',
        desc: '软装 4,000–4,500 元/㎡ 真实成交标准。从家具选型到饰品陈设，提供完整软装清单与采购指导。',
        backup: '完整软装清单 · 采购指导。',
      },
      {
        icon: '🗝️',
        title: '整装服务',
        subtitle: 'Turnkey Package',
        desc: '设计、施工、软装一站式交付。一个团队全程负责，预算锁定、工期锁定，交付即可入住。',
        backup: '预算锁定 · 工期锁定',
      },
    ],
  },
  cases: {
    eyebrow: 'Selected Works',
    title: '真实落地，有据可查。',
    sub: 'Real Projects. Proven Results.',
    photoBadge: (n: number) => `${n} 张`,
    comingSoon: '实景照片即将上线',
    moreStyle: '更多风格',
    spaceTitle: '空间构成',
    stats: (c: number, p: number) => `共 ${c} 个案例 · ${p} 张实景照片`,
  },
  styles: {
    全部: '全部',
    法式: '法式',
    现代: '现代',
    侘寂: '侘寂',
    意式极简: '意式极简',
    现代奶油: '现代奶油',
    法式轻奢: '法式轻奢',
    现代小法: '现代小法',
    更多: '更多',
  } as Record<string, string>,
  rooms: {
    living: '客厅',
    dining: '餐厅',
    kitchen: '厨房',
    bedroom: '卧室',
    kids_room: '儿童房',
    bathroom: '卫浴',
    study: '书房',
    tea_room: '茶室',
    recreation: '娱乐区',
    staircase: '楼梯间',
    bar: '水吧',
    plan: '平面图',
  } as Record<RoomType, string>,
  pricing: {
    eyebrow: 'Transparent Pricing',
    title: '报价，基于真实成交数据。',
    body: '我们不凭空估价。每一个报价都来自案例库的真实单方造价，按风格、地区、档次系数校准：',
    formula: '总价 = 面积 × 单方造价 × 风格系数 × 地区系数 × 档次系数',
    hardCost: '硬装单方造价',
    softCost: '软装单方造价',
    note: '数据来源：派尚设计真实成交项目 · 展示层 USD/IDR 换算即将上线',
    materialsLink: '查看印尼本地材料库 →',
  },
  materialsPage: {
    eyebrow: 'Material Library',
    title: '印尼本地材料 SKU 库',
    sub: 'BOM 精确报价的数据底座 · 8 大类 · 3 档',
    disclaimer: '价格为雅加达公开零售参考价，最终报价以供应商确认为准。',
    all: '全部',
    unitLabel: '单价',
    laborLabel: '安装人工',
    includedLabel: '包工包料',
    supplierLabel: '供应商',
    updatedLabel: '更新',
    stats: (n: number, c: number) => `共 ${n} 个 SKU · ${c} 个大类`,
    categories: {
      石材: '石材',
      瓷砖: '瓷砖',
      木地板与木饰面: '木地板与木饰面',
      涂料与微水泥: '涂料与微水泥',
      卫浴: '卫浴',
      门窗五金: '门窗五金',
      灯具电气: '灯具电气',
      定制柜软装: '定制柜软装',
    } as Record<string, string>,
    tiers: { standard: '标准', luxury: '豪华', ultra: '顶级' } as Record<string, string>,
  },
  bom: {
    tabEstimate: '初步估算',
    tabBom: 'BOM 精报',
    thQty: '数量',
    thUnitPrice: '单价',
    thSubtotal: '小计',
    labor: '安装人工（×1.4 施工加权）',
    total: 'BOM 合计',
    anchorHigher: (p: number) => `与初步估算的装修分项相比高 ${p}%`,
    anchorLower: (p: number) => `与初步估算的装修分项相比低 ${Math.abs(p)}%`,
    surveyHint: '与估算装修分项差异超过 25%，建议设计师上门量房复核。',
    disclaimer: '价格基于雅加达公开零售参考价 + 标准算量规则，最终以设计师量房与供应商确认为准。',
    extrasLabel: '附加项',
    itemCount: (n: number) => `${n} 项`,
    scopes: {
      全屋地面: '全屋地面',
      卧室地面: '卧室地面',
      全屋墙面: '全屋墙面',
      客厅背景墙: '客厅背景墙',
      各房间门: '各房间门',
      全屋照明: '全屋照明',
      卧室衣柜: '卧室衣柜',
      厨房厨柜: '厨房厨柜',
      '卫浴（估算）': '卫浴（估算）',
      户外: '户外',
    } as Record<string, string>,
  },
  team: {
    eyebrow: 'Our Team',
    title: '专注豪宅的设计与营造团队',
    body: '我们是一支深耕中国顶级豪宅的设计团队，长期为中国精英人士打造理想居所，沉淀了成熟的设计体系与营造经验。我们同时拥有业内最优秀的施工技术产业工人团队，以毫米级的工艺标准兑现设计承诺。今天，我们把这份经验与匠心带到印尼，致力于为高净值人士营造美好居所。',
    metrics: [
      { value: '100+', label: '服务精英家庭' },
      { value: '25', label: '落地案例（套）' },
      { value: '自有', label: '产业工人团队' },
    ],
  },
  estimate: {
    eyebrow: 'Instant Estimate',
    title: '上传户型图，30 分钟获取初步方案与估价',
    body: '无需注册。支持 JPG / PNG / PDF / DXF，AI 自动识别空间结构并匹配最佳案例。',
    cta: '免费快速估价',
  },
  booking: {
    title: '预约设计师',
    sub: 'Book a Designer',
    body: '我们的设计师将在 24 小时内与您联系。初步估价与方案展示完全免费。',
    cta: '立即预约',
  },
  quote: {
    eyebrow: 'Instant Estimate',
    title: '快速估价',
    pageSub: '填写以下信息，即时获得初步估价。',
    uploadLabel: '户型图 / 照片（选填）',
    uploadHint: '文件不会上传，仅本地预览。',
    uploadButton: '选择文件（JPG / PNG / PDF / DXF，单文件 ≤10MB）',
    fileTooBig: '文件过大，请上传 10MB 以内的文件',
    styleLabel: '偏好风格',
    areaLabel: '面积（㎡）',
    areaPlaceholder: '如 300',
    areaError: '请输入 50–5000 之间的面积。',
    roomsLabel: '房间数',
    floorsLabel: '楼层数',
    tierLabel: '档次',
    tiers: { standard: '标准', luxury: '豪华', ultra: '顶级' },
    regionLabel: '项目地区',
    regions: { jakarta: '雅加达', bali: '巴厘岛', surabaya: '泗水', other: '其他' },
    extrasLabel: '附加项',
    pool: '泳池（+8%）',
    garden: '花园（+5%）',
    resultTitle: '初步估价',
    resultHint: '填写有效面积后即时生成估价。',
    breakdownTitle: '费用构成',
    breakdown: {
      structure: '结构',
      fitout: '装修',
      mep: '机电',
      landscape: '景观',
      furniture: '家具',
      design: '设计费',
    },
    refCase: (name: string) => `参考案例：${name}`,
    viewCase: '查看案例 →',
    disclaimer: '初步估算仅供参考，最终以设计师上门量房为准。',
    ctaBook: '预约设计师深化方案 →',
  },
  design: {
    title: 'AI 设计建议',
    loading: 'AI 正在生成设计建议…',
    failed: '设计建议暂时不可用',
  },
  dxf: {
    parsed: (n: number, area: number) =>
      `识别到 ${n} 个房间，总面积 ${area} ㎡ 已自动填入（可编辑）`,
    failed: '无法解析该 DXF 文件',
  },
  detail: {
    back: '← 返回案例库',
    allSpaces: '全部空间',
    likeTitle: '喜欢这种风格？',
  },
  footer: {
    tagline: '找到更好的方式，建造更美的空间。',
    taglineSub: 'Find a better way. Build a beautiful space.',
    related: '相关项目：IndoScout-D-B（获客端）',
  },
  bookingForm: {
    eyebrow: 'Booking',
    title: '预约设计师',
    waHint: '提交后将跳转到 WhatsApp 与我们的设计顾问对话。',
    name: '姓名',
    whatsapp: 'WhatsApp 号',
    email: '邮箱（选填）',
    location: '项目所在地',
    locationPlaceholder: '如 雅加达',
    area: '预估面积（㎡）',
    style: '偏好风格',
    message: '留言（选填）',
    submit: '通过 WhatsApp 提交',
    required: '请填写姓名和 WhatsApp 号。',
  },
  estimateDemo: {
    eyebrow: 'Internal Demo · P1 quick_estimate',
    badge: '内部演示版 · 数据校对中（draft）',
    title: '工序级快速估价引擎',
    pageSub:
      '自下而上合成：风格配置 → 工序展开（含强制前置）→ KG 工时 × 人工费率 + Atelier SKU 材料价。本页为内部演示，全部数字尚未经真实项目校对，不对外承诺。',
    styleLabel: '装修风格',
    styleNames: {
      french: '法式',
      modern: '现代',
      'wabi-sabi': '侘寂',
      'italian-minimal': '意式极简',
      'modern-cream': '现代奶油',
    } as Record<string, string>,
    areaLabel: '建筑面积（㎡，50–500）',
    spacesLabel: '空间（多选）',
    spaceNames: {
      living: '客厅',
      dining: '餐厅',
      kitchen: '厨房',
      bedroom: '卧室',
      bathroom: '卫浴',
      study: '书房',
    } as Record<string, string>,
    locationLabel: '城市',
    locationNames: { jakarta: '雅加达', bali: '巴厘岛' } as Record<string, string>,
    tierLabel: '档次（P1 暂不分档定价）',
    tierNames: { standard: '标准', premium: '轻奢', luxury: '豪华' } as Record<string, string>,
    ramadanLabel: '斋月施工（工期 ×1.3）',
    submit: '生成估价',
    resultHint: '填写左侧信息并点击「生成估价」。',
    totalRange: '总价区间（IDR）',
    perSqm: '单方造价（IDR/㎡）',
    timelineTitle: '工期估算（天）',
    timelineMin: '最快',
    timelineLikely: '预期',
    timelineMax: '最慢',
    crewTitle: '班组配置建议',
    crewDays: (d: number) => `${d} 工日`,
    breakdownTitle: '逐工序明细',
    colSpace: '空间',
    colElement: '部位 / 工序',
    colQty: '工程量',
    colMaterial: '材料费',
    colLabor: '人工费',
    colAux: '辅助费',
    colDays: '工期(天)',
    colCrew: '班组',
    prereqMark: '（前置）',
    gapsTitle: '数据缺口警示 —— 以下环节走了默认值或未校对',
    gapsSub: '如实列出是刻意为之：这些数字校对前，估价仅供参考。',
    confidenceLabel: '置信度',
    confidenceLow: '低（含 draft 数据缺口）',
    confidenceMedium: '中',
    usdRef: 'USD 参考（IDR/USD 15000）',
    errorGeneric: '输入有误：',
  },
};

const en: typeof zh = {
  nav: {
    services: 'Services',
    cases: 'Works',
    pricing: 'Pricing',
    team: 'Team',
    materials: 'Materials',
    book: 'Book a Designer',
  },
  hero: {
    eyebrow: 'Nusantara Atelier · Luxury Design-and-Build Studio',
    titleA: 'From Design to Delivery —',
    titleB: 'Done Right, Once.',
    tagline: '从设计到呈现，一步到位。',
    desc: "China's top-tier residential expertise, now serving Indonesia's luxury villa market.",
    descChain: 'Interior Design · Precision Fit-Out · Furniture & Soft Furnishing',
    ctaPrimary: 'Book a Designer',
    ctaSecondary: 'Explore Our Work',
    scroll: 'Scroll to explore ↓',
  },
  services: {
    eyebrow: 'Our Services',
    title: 'One Complete Service Chain',
    items: [
      {
        icon: '🏛️',
        title: 'Interior Design',
        subtitle: '室内设计',
        desc: 'Style systems backed by real completed projects: French, Modern, Wabi-Sabi, Italian Minimalist, Cream Modern, French Luxury.',
        backup: 'Every proposal is grounded in actual delivered work.',
      },
      {
        icon: '🔨',
        title: 'Precision Fit-Out',
        subtitle: '精密装修',
        desc: 'Transparent hard-fit-out pricing per square meter, benchmarked at RMB 4,500–5,500/㎡ from real projects.',
        backup: "Matching China's first-tier luxury craftsmanship.",
      },
      {
        icon: '🛋️',
        title: 'Furniture & Soft Furnishing',
        subtitle: '家具软装指导',
        desc: 'Soft furnishing at RMB 4,000–4,500/㎡ based on real transactions, with full furniture selection lists and procurement guidance.',
        backup: 'Full furnishing list · Procurement guidance.',
      },
      {
        icon: '🗝️',
        title: 'Turnkey Package',
        subtitle: '整装服务',
        desc: 'One team, one contract, one locked budget — design, fit-out, and furnishing delivered turnkey. Move in on handover day.',
        backup: 'Locked Budget · Locked Timeline',
      },
    ],
  },
  cases: {
    eyebrow: 'Selected Works',
    title: 'Real Projects. Proven Results.',
    sub: '真实落地，有据可查。',
    photoBadge: (n: number) => `${n} Photos`,
    comingSoon: 'Photos coming soon',
    moreStyle: 'More Styles',
    spaceTitle: 'Spatial Composition',
    stats: (c: number, p: number) => `${c} Projects · ${p} Photos`,
  },
  styles: {
    全部: 'All',
    法式: 'French',
    现代: 'Modern',
    侘寂: 'Wabi-Sabi',
    意式极简: 'Italian Minimalist',
    现代奶油: 'Cream Modern',
    法式轻奢: 'French Luxury',
    现代小法: 'Modern French',
    更多: 'More',
  } as Record<string, string>,
  rooms: {
    living: 'Living Room',
    dining: 'Dining Room',
    kitchen: 'Kitchen',
    bedroom: 'Bedroom',
    kids_room: 'Kids Room',
    bathroom: 'Bathroom',
    study: 'Study',
    tea_room: 'Tea Room',
    recreation: 'Recreation',
    staircase: 'Staircase',
    bar: 'Bar',
    plan: 'Floor Plan',
  } as Record<RoomType, string>,
  pricing: {
    eyebrow: 'Transparent Pricing',
    title: 'Pricing Backed by Real Transaction Data.',
    body: 'We never quote from thin air. Every estimate derives from actual per-sqm costs in our case library, calibrated by style, location, and tier:',
    formula: 'Total = Area × Unit Cost × Style Factor × Location Factor × Tier Factor',
    hardCost: 'Hard Fit-Out',
    softCost: 'Soft Furnishing',
    note: 'Source: real transactions by Paishang Design · USD/IDR conversion coming soon',
    materialsLink: 'Browse the Indonesia Material Library →',
  },
  materialsPage: {
    eyebrow: 'Material Library',
    title: 'Indonesia Local Material SKU Library',
    sub: 'The data backbone of BOM-precision quoting · 8 categories · 3 tiers',
    disclaimer: 'Prices are public retail references for Jakarta; final quotes are subject to supplier confirmation.',
    all: 'All',
    unitLabel: 'Unit Price',
    laborLabel: 'Installation Labor',
    includedLabel: 'Supply & Install',
    supplierLabel: 'Supplier',
    updatedLabel: 'Updated',
    stats: (n: number, c: number) => `${n} SKUs · ${c} Categories`,
    categories: {
      石材: 'Stone',
      瓷砖: 'Tiles',
      木地板与木饰面: 'Wood Flooring & Panels',
      涂料与微水泥: 'Paint & Microcement',
      卫浴: 'Sanitary',
      门窗五金: 'Doors, Windows & Hardware',
      灯具电气: 'Lighting & Electrical',
      定制柜软装: 'Custom Cabinetry & Curtains',
    } as Record<string, string>,
    tiers: { standard: 'Standard', luxury: 'Luxury', ultra: 'Ultra-Luxury' } as Record<string, string>,
  },
  bom: {
    tabEstimate: 'Quick Estimate',
    tabBom: 'Precise BOM',
    thQty: 'Qty',
    thUnitPrice: 'Unit Price',
    thSubtotal: 'Subtotal',
    labor: 'Installation Labor (×1.4 construction weight)',
    total: 'BOM Total',
    anchorHigher: (p: number) => `${p}% above the fit-out portion of the quick estimate`,
    anchorLower: (p: number) => `${Math.abs(p)}% below the fit-out portion of the quick estimate`,
    surveyHint: 'Difference from the estimated fit-out portion exceeds 25% — an on-site survey by our designer is recommended.',
    disclaimer: 'Prices are based on Jakarta public retail references and standard quantity rules; the final quote follows an on-site survey and supplier confirmation.',
    extrasLabel: 'Extras',
    itemCount: (n: number) => `${n} items`,
    scopes: {
      全屋地面: 'Whole-house flooring',
      卧室地面: 'Bedroom flooring',
      全屋墙面: 'Whole-house walls',
      客厅背景墙: 'Living-room feature wall',
      各房间门: 'Room doors',
      全屋照明: 'Whole-house lighting',
      卧室衣柜: 'Bedroom wardrobes',
      厨房厨柜: 'Kitchen cabinetry',
      '卫浴（估算）': 'Bathrooms (estimated)',
      户外: 'Outdoor',
    } as Record<string, string>,
  },
  team: {
    eyebrow: 'Our Team',
    title: 'A Team Devoted to Luxury Living',
    body: "We are a design team rooted in China's top-tier luxury residences, long trusted by China's elite to craft their ideal homes — backed by a mature design system and proven delivery experience. Our in-house craftsmen are among the finest in the industry, honoring every design promise with millimeter-level precision. Today, we bring this experience and dedication to Indonesia, committed to creating beautiful homes for high-net-worth families.",
    metrics: [
      { value: '100+', label: 'Elite Families Served' },
      { value: '25', label: 'Completed Projects' },
      { value: 'In-house', label: 'Craftsmen Team' },
    ],
  },
  estimate: {
    eyebrow: 'Instant Estimate',
    title: 'Upload Your Floor Plan. Get a First Proposal in 30 Minutes.',
    body: 'No registration needed. JPG / PNG / PDF / DXF supported — AI parses your space and matches the best reference case.',
    cta: 'Free Instant Estimate',
  },
  booking: {
    title: 'Book a Designer',
    sub: '预约设计师',
    body: 'Our designers will reach out within 24 hours. First estimate and proposal are completely free.',
    cta: 'Book Now',
  },
  quote: {
    eyebrow: 'Instant Estimate',
    title: 'Instant Estimate',
    pageSub: 'Fill in the details below for an instant ballpark estimate.',
    uploadLabel: 'Floor Plan / Photos (optional)',
    uploadHint: 'File stays on your device — local preview only.',
    uploadButton: 'Choose File (JPG / PNG / PDF / DXF, max 10MB each)',
    fileTooBig: 'File too large, max 10MB',
    styleLabel: 'Preferred Style',
    areaLabel: 'Area (㎡)',
    areaPlaceholder: 'e.g. 300',
    areaError: 'Please enter an area between 50 and 5,000 ㎡.',
    roomsLabel: 'Rooms',
    floorsLabel: 'Floors',
    tierLabel: 'Tier',
    tiers: { standard: 'Standard', luxury: 'Luxury', ultra: 'Ultra-Luxury' },
    regionLabel: 'Project Region',
    regions: { jakarta: 'Jakarta', bali: 'Bali', surabaya: 'Surabaya', other: 'Other' },
    extrasLabel: 'Extras',
    pool: 'Swimming Pool (+8%)',
    garden: 'Garden (+5%)',
    resultTitle: 'Estimated Total',
    resultHint: 'Enter a valid area to see the instant estimate.',
    breakdownTitle: 'Cost Breakdown',
    breakdown: {
      structure: 'Structure',
      fitout: 'Fit-Out',
      mep: 'MEP',
      landscape: 'Landscape',
      furniture: 'Furniture',
      design: 'Design Fee',
    },
    refCase: (name: string) => `Reference: ${name}`,
    viewCase: 'View Case →',
    disclaimer: 'For reference only — the final quote follows an on-site survey by our designer.',
    ctaBook: 'Book a Designer to Refine →',
  },
  design: {
    title: 'AI Design Suggestion',
    loading: 'AI is generating design suggestions…',
    failed: 'Design suggestions unavailable',
  },
  dxf: {
    parsed: (n: number, area: number) =>
      `${n} rooms detected — total area ${area} ㎡ filled in (editable)`,
    failed: 'Could not parse this DXF file',
  },
  detail: {
    back: '← Back to Works',
    allSpaces: 'All Spaces',
    likeTitle: 'Like This Style?',
  },
  footer: {
    tagline: 'Find a better way. Build a beautiful space.',
    taglineSub: '找到更好的方式，建造更美的空间。',
    related: 'Related: IndoScout-D-B (Lead Generation)',
  },
  bookingForm: {
    eyebrow: 'Booking',
    title: 'Book a Designer',
    waHint: "You'll be redirected to WhatsApp to chat with our design consultant.",
    name: 'Name',
    whatsapp: 'WhatsApp Number',
    email: 'Email (optional)',
    location: 'Project Location',
    locationPlaceholder: 'e.g. Jakarta',
    area: 'Estimated Area (㎡)',
    style: 'Preferred Style',
    message: 'Message (optional)',
    submit: 'Submit via WhatsApp',
    required: 'Please fill in your name and WhatsApp number.',
  },
  estimateDemo: {
    eyebrow: 'Internal Demo · P1 quick_estimate',
    badge: 'Internal demo · data under review (draft)',
    title: 'Process-level Quick Estimate Engine',
    pageSub:
      'Bottom-up synthesis: style config → process expansion (incl. mandatory prerequisites) → KG work-hours × labor rates + Atelier SKU material prices. Internal demo only — all figures are unverified and not a public commitment.',
    styleLabel: 'Style',
    styleNames: {
      french: 'French',
      modern: 'Modern',
      'wabi-sabi': 'Wabi-Sabi',
      'italian-minimal': 'Italian Minimal',
      'modern-cream': 'Modern Cream',
    } as Record<string, string>,
    areaLabel: 'Floor area (㎡, 50–500)',
    spacesLabel: 'Spaces (multi-select)',
    spaceNames: {
      living: 'Living',
      dining: 'Dining',
      kitchen: 'Kitchen',
      bedroom: 'Bedroom',
      bathroom: 'Bathroom',
      study: 'Study',
    } as Record<string, string>,
    locationLabel: 'City',
    locationNames: { jakarta: 'Jakarta', bali: 'Bali' } as Record<string, string>,
    tierLabel: 'Tier (P1: not yet priced by tier)',
    tierNames: { standard: 'Standard', premium: 'Premium', luxury: 'Luxury' } as Record<string, string>,
    ramadanLabel: 'Ramadan schedule (timeline ×1.3)',
    submit: 'Run Estimate',
    resultHint: 'Fill in the form and press "Run Estimate".',
    totalRange: 'Total range (IDR)',
    perSqm: 'Per-sqm (IDR/㎡)',
    timelineTitle: 'Timeline (days)',
    timelineMin: 'Min',
    timelineLikely: 'Likely',
    timelineMax: 'Max',
    crewTitle: 'Crew Plan',
    crewDays: (d: number) => `${d} man-days`,
    breakdownTitle: 'Process Breakdown',
    colSpace: 'Space',
    colElement: 'Element / Process',
    colQty: 'Qty',
    colMaterial: 'Material',
    colLabor: 'Labor',
    colAux: 'Aux',
    colDays: 'Days',
    colCrew: 'Crew',
    prereqMark: ' (prereq)',
    gapsTitle: 'Data Gaps — defaults or unverified figures used',
    gapsSub: 'Listed honestly on purpose: until verified, estimates are indicative only.',
    confidenceLabel: 'Confidence',
    confidenceLow: 'Low (draft data gaps)',
    confidenceMedium: 'Medium',
    usdRef: 'USD ref (IDR/USD 15000)',
    errorGeneric: 'Invalid input: ',
  },
};

const id: typeof zh = {
  nav: {
    services: 'Layanan',
    cases: 'Proyek',
    pricing: 'Harga',
    team: 'Tim',
    materials: 'Material',
    book: 'Jadwalkan Desainer',
  },
  hero: {
    eyebrow: 'Nusantara Atelier · Studio Desain & Bangun Mewah',
    titleA: 'Dari Desain hingga Serah Terima —',
    titleB: 'Sekali Jadi.',
    tagline: 'Dari desain hingga serah terima, beres sekaligus.',
    desc: 'Keahlian hunian mewah kelas satu dari Tiongkok, kini melayani pasar vila premium Indonesia.',
    descChain: 'Desain Interior · Fit-Out Presisi · Furnitur & Dekorasi',
    ctaPrimary: 'Jadwalkan Desainer',
    ctaSecondary: 'Jelajahi Karya Kami',
    scroll: 'Gulir untuk menjelajah ↓',
  },
  services: {
    eyebrow: 'Our Services',
    title: 'Satu Rangkaian Layanan yang Utuh',
    items: [
      {
        icon: '🏛️',
        title: 'Desain Interior',
        subtitle: 'Interior Design',
        desc: 'Sistem gaya yang didukung proyek nyata: French, Modern, Wabi-Sabi, Italian Minimalist, Cream Modern, French Luxury.',
        backup: 'Setiap proposal berpijak pada karya yang benar-benar terkirim.',
      },
      {
        icon: '🔨',
        title: 'Fit-Out Presisi',
        subtitle: 'Precision Fit-Out',
        desc: 'Harga fit-out transparan hingga per meter persegi, mengacu pada biaya riil RMB 4.500–5.500/㎡ dari proyek nyata.',
        backup: 'Setara kualitas hunian mewah lapis pertama Tiongkok.',
      },
      {
        icon: '🛋️',
        title: 'Furnitur & Dekorasi',
        subtitle: 'Furniture & Soft Furnishing',
        desc: 'Furnishing RMB 4.000–4.500/㎡ berdasarkan transaksi nyata, lengkap dengan daftar seleksi furnitur dan panduan pengadaan.',
        backup: 'Daftar furnishing lengkap · Panduan pengadaan.',
      },
      {
        icon: '🗝️',
        title: 'Layanan Turnkey',
        subtitle: 'Turnkey Package',
        desc: 'Satu tim, satu kontrak, satu anggaran terkunci — desain, fit-out, dan furnishing diserahkan turnkey. Langsung huni saat serah terima.',
        backup: 'Anggaran & Jadwal Terkunci',
      },
    ],
  },
  cases: {
    eyebrow: 'Selected Works',
    title: 'Proyek Nyata. Hasil Terbukti.',
    sub: 'Real Projects. Proven Results.',
    photoBadge: (n: number) => `${n} Foto`,
    comingSoon: 'Foto segera hadir',
    moreStyle: 'Gaya Lainnya',
    spaceTitle: 'Komposisi Ruang',
    stats: (c: number, p: number) => `${c} Proyek · ${p} Foto`,
  },
  styles: {
    全部: 'Semua',
    法式: 'Prancis',
    现代: 'Modern',
    侘寂: 'Wabi-Sabi',
    意式极简: 'Minimalis Italia',
    现代奶油: 'Modern Krem',
    法式轻奢: 'French Luxury',
    现代小法: 'Modern Prancis',
    更多: 'Lainnya',
  } as Record<string, string>,
  rooms: {
    living: 'Ruang Keluarga',
    dining: 'Ruang Makan',
    kitchen: 'Dapur',
    bedroom: 'Kamar Tidur',
    kids_room: 'Kamar Anak',
    bathroom: 'Kamar Mandi',
    study: 'Ruang Kerja',
    tea_room: 'Ruang Teh',
    recreation: 'Area Rekreasi',
    staircase: 'Tangga',
    bar: 'Bar',
    plan: 'Denah',
  } as Record<RoomType, string>,
  pricing: {
    eyebrow: 'Transparent Pricing',
    title: 'Harga Berdasarkan Data Transaksi Nyata.',
    body: 'Kami tidak pernah menebak-nebak harga. Setiap estimasi berasal dari biaya per m² riil dalam pustaka kasus kami, dikalibrasi menurut gaya, lokasi, dan kelas:',
    formula: 'Total = Luas × Biaya per m² × Faktor Gaya × Faktor Lokasi × Faktor Kelas',
    hardCost: 'Fit-Out',
    softCost: 'Furnishing',
    note: 'Sumber: transaksi nyata Paishang Design · Konversi USD/IDR segera hadir',
    materialsLink: 'Lihat Pustaka Material Indonesia →',
  },
  materialsPage: {
    eyebrow: 'Material Library',
    title: 'Pustaka SKU Material Lokal Indonesia',
    sub: 'Tulang punggung data untuk penawaran BOM presisi · 8 kategori · 3 kelas',
    disclaimer: 'Harga adalah referensi ritel publik Jakarta; penawaran final mengikuti konfirmasi pemasok.',
    all: 'Semua',
    unitLabel: 'Harga Satuan',
    laborLabel: 'Upah Pasang',
    includedLabel: 'Termasuk Pasang',
    supplierLabel: 'Pemasok',
    updatedLabel: 'Diperbarui',
    stats: (n: number, c: number) => `${n} SKU · ${c} Kategori`,
    categories: {
      石材: 'Batu Alam',
      瓷砖: 'Keramik & Granit',
      木地板与木饰面: 'Lantai Kayu & Panel Kayu',
      涂料与微水泥: 'Cat & Microcement',
      卫浴: 'Sanitair',
      门窗五金: 'Pintu, Jendela & Hardware',
      灯具电气: 'Lampu & Elektrikal',
      定制柜软装: 'Kabinet Kustom & Tirai',
    } as Record<string, string>,
    tiers: { standard: 'Standar', luxury: 'Mewah', ultra: 'Ultra-Mewah' } as Record<string, string>,
  },
  bom: {
    tabEstimate: 'Estimasi Cepat',
    tabBom: 'BOM Presisi',
    thQty: 'Jumlah',
    thUnitPrice: 'Harga Satuan',
    thSubtotal: 'Subtotal',
    labor: 'Upah Pemasangan (bobot konstruksi ×1.4)',
    total: 'Total BOM',
    anchorHigher: (p: number) => `${p}% di atas komponen fit-out dari estimasi cepat`,
    anchorLower: (p: number) => `${Math.abs(p)}% di bawah komponen fit-out dari estimasi cepat`,
    surveyHint: 'Selisih dengan komponen fit-out estimasi melebihi 25% — disarankan pengukuran langsung oleh desainer kami.',
    disclaimer: 'Harga berdasarkan referensi ritel publik Jakarta dan aturan perhitungan standar; penawaran final mengikuti pengukuran langsung dan konfirmasi pemasok.',
    extrasLabel: 'Tambahan',
    itemCount: (n: number) => `${n} item`,
    scopes: {
      全屋地面: 'Lantai seluruh rumah',
      卧室地面: 'Lantai kamar tidur',
      全屋墙面: 'Dinding seluruh rumah',
      客厅背景墙: 'Dinding fokus ruang keluarga',
      各房间门: 'Pintu tiap ruangan',
      全屋照明: 'Pencahayaan seluruh rumah',
      卧室衣柜: 'Lemari kamar tidur',
      厨房厨柜: 'Kabinet dapur',
      '卫浴（估算）': 'Kamar mandi (estimasi)',
      户外: 'Area luar',
    } as Record<string, string>,
  },
  team: {
    eyebrow: 'Our Team',
    title: 'Tim yang Berdedikasi untuk Hunian Mewah',
    body: 'Kami adalah tim desain yang berakar pada hunian mewah kelas atas Tiongkok, lama dipercaya para elite Tiongkok untuk mewujudkan hunian ideal — didukung sistem desain yang matang dan pengalaman terbukti. Tim pengrajin internal kami termasuk yang terbaik di industri, menepati setiap janji desain dengan presisi milimeter. Kini, kami membawa pengalaman dan dedikasi ini ke Indonesia, berkomitmen menciptakan hunian indah bagi keluarga high-net-worth.',
    metrics: [
      { value: '100+', label: 'Keluarga Elite Terlayani' },
      { value: '25', label: 'Proyek Selesai' },
      { value: 'Internal', label: 'Tim Pengrajin' },
    ],
  },
  estimate: {
    eyebrow: 'Instant Estimate',
    title: 'Unggah Denah Anda. Dapatkan Proposal Awal dalam 30 Menit.',
    body: 'Tanpa registrasi. Mendukung JPG / PNG / PDF / DXF — AI membaca struktur ruang Anda dan mencocokkan kasus referensi terbaik.',
    cta: 'Estimasi Gratis',
  },
  booking: {
    title: 'Jadwalkan Desainer',
    sub: 'Book a Designer',
    body: 'Desainer kami akan menghubungi Anda dalam 24 jam. Estimasi pertama dan presentasi proposal sepenuhnya gratis.',
    cta: 'Jadwalkan Sekarang',
  },
  quote: {
    eyebrow: 'Instant Estimate',
    title: 'Estimasi Instan',
    pageSub: 'Isi detail berikut untuk estimasi awal seketika.',
    uploadLabel: 'Denah / Foto (opsional)',
    uploadHint: 'File tidak diunggah — hanya pratinjau lokal di perangkat Anda.',
    uploadButton: 'Pilih File (JPG / PNG / PDF / DXF, maks. 10MB per file)',
    fileTooBig: 'File terlalu besar, maksimal 10MB',
    styleLabel: 'Gaya Pilihan',
    areaLabel: 'Luas (㎡)',
    areaPlaceholder: 'mis. 300',
    areaError: 'Masukkan luas antara 50–5.000 ㎡.',
    roomsLabel: 'Jumlah Kamar',
    floorsLabel: 'Jumlah Lantai',
    tierLabel: 'Kelas',
    tiers: { standard: 'Standar', luxury: 'Mewah', ultra: 'Ultra-Mewah' },
    regionLabel: 'Lokasi Proyek',
    regions: { jakarta: 'Jakarta', bali: 'Bali', surabaya: 'Surabaya', other: 'Lainnya' },
    extrasLabel: 'Fasilitas Tambahan',
    pool: 'Kolam Renang (+8%)',
    garden: 'Taman (+5%)',
    resultTitle: 'Estimasi Total',
    resultHint: 'Masukkan luas yang valid untuk melihat estimasi.',
    breakdownTitle: 'Rincian Biaya',
    breakdown: {
      structure: 'Struktur',
      fitout: 'Fit-Out',
      mep: 'Mekanikal & Elektrikal',
      landscape: 'Lanskap',
      furniture: 'Furnitur',
      design: 'Biaya Desain',
    },
    refCase: (name: string) => `Referensi: ${name}`,
    viewCase: 'Lihat Proyek →',
    disclaimer: 'Estimasi awal hanya sebagai referensi — hasil akhir mengikuti pengukuran langsung oleh desainer kami.',
    ctaBook: 'Jadwalkan Desainer untuk Detail →',
  },
  design: {
    title: 'Saran Desain AI',
    loading: 'AI sedang membuat saran desain…',
    failed: 'Saran desain tidak tersedia',
  },
  dxf: {
    parsed: (n: number, area: number) =>
      `${n} ruangan terdeteksi — total luas ${area} ㎡ terisi otomatis (dapat diubah)`,
    failed: 'File DXF tidak dapat diproses',
  },
  detail: {
    back: '← Kembali ke Proyek',
    allSpaces: 'Semua Ruang',
    likeTitle: 'Suka Gaya Ini?',
  },
  footer: {
    tagline: 'Temukan cara yang lebih baik. Bangun ruang yang indah.',
    taglineSub: 'Find a better way. Build a beautiful space.',
    related: 'Proyek terkait: IndoScout-D-B (Akuisisi Klien)',
  },
  bookingForm: {
    eyebrow: 'Booking',
    title: 'Jadwalkan Desainer',
    waHint: 'Anda akan diarahkan ke WhatsApp untuk berbicara dengan konsultan desain kami.',
    name: 'Nama',
    whatsapp: 'Nomor WhatsApp',
    email: 'Email (opsional)',
    location: 'Lokasi Proyek',
    locationPlaceholder: 'mis. Jakarta',
    area: 'Perkiraan Luas (㎡)',
    style: 'Gaya Pilihan',
    message: 'Pesan (opsional)',
    submit: 'Kirim via WhatsApp',
    required: 'Mohon isi nama dan nomor WhatsApp Anda.',
  },
  estimateDemo: {
    eyebrow: 'Demo Internal · P1 quick_estimate',
    badge: 'Demo internal · data sedang diverifikasi (draft)',
    title: 'Mesin Estimasi Cepat per Proses',
    pageSub:
      'Sintesis bottom-up: konfigurasi gaya → ekspansi proses (termasuk prasyarat wajib) → jam kerja KG × tarif tenaga + harga material SKU Atelier. Khusus demo internal — semua angka belum diverifikasi dan bukan komitmen publik.',
    styleLabel: 'Gaya',
    styleNames: {
      french: 'Prancis',
      modern: 'Modern',
      'wabi-sabi': 'Wabi-Sabi',
      'italian-minimal': 'Minimalis Italia',
      'modern-cream': 'Krem Modern',
    } as Record<string, string>,
    areaLabel: 'Luas bangunan (㎡, 50–500)',
    spacesLabel: 'Ruang (multi-pilih)',
    spaceNames: {
      living: 'Ruang tamu',
      dining: 'Ruang makan',
      kitchen: 'Dapur',
      bedroom: 'Kamar tidur',
      bathroom: 'Kamar mandi',
      study: 'Ruang kerja',
    } as Record<string, string>,
    locationLabel: 'Kota',
    locationNames: { jakarta: 'Jakarta', bali: 'Bali' } as Record<string, string>,
    tierLabel: 'Tier (P1: belum dihargai per tier)',
    tierNames: { standard: 'Standar', premium: 'Premium', luxury: 'Mewah' } as Record<string, string>,
    ramadanLabel: 'Jadwal Ramadan (timeline ×1,3)',
    submit: 'Hitung Estimasi',
    resultHint: 'Isi formulir lalu tekan "Hitung Estimasi".',
    totalRange: 'Rentang total (IDR)',
    perSqm: 'Per meter persegi (IDR/㎡)',
    timelineTitle: 'Estimasi waktu (hari)',
    timelineMin: 'Min',
    timelineLikely: 'Perkiraan',
    timelineMax: 'Maks',
    crewTitle: 'Rencana Tim Kerja',
    crewDays: (d: number) => `${d} hari kerja`,
    breakdownTitle: 'Rincian per Proses',
    colSpace: 'Ruang',
    colElement: 'Elemen / Proses',
    colQty: 'Qty',
    colMaterial: 'Material',
    colLabor: 'Tenaga',
    colAux: 'Bantu',
    colDays: 'Hari',
    colCrew: 'Tim',
    prereqMark: ' (prasyarat)',
    gapsTitle: 'Celah Data — angka default / belum diverifikasi',
    gapsSub: 'Ditampilkan apa adanya dengan sengaja: sebelum diverifikasi, estimasi bersifat indikatif.',
    confidenceLabel: 'Tingkat keyakinan',
    confidenceLow: 'Rendah (celah data draft)',
    confidenceMedium: 'Sedang',
    usdRef: 'Ref USD (IDR/USD 15000)',
    errorGeneric: 'Input tidak valid: ',
  },
};

export type Dictionary = typeof zh;

export const dictionaries: Record<Locale, Dictionary> = { zh, en, id };

/** 首次访问语言探测：zh* → 中文，en* → English，其余（含 id*）→ 印尼语（市场优先） */
export function detectLocale(navLang: string): Locale {
  const lang = navLang.toLowerCase();
  if (lang.startsWith('zh')) return 'zh';
  if (lang.startsWith('en')) return 'en';
  return 'id';
}
